...
class ZipkinSelfTracingPostgreSQLStorageConfiguration extends DefaultExecuteListener {

  @Autowired ZipkinPostgreSQLStorageProperties props;
  @Autowired CurrentTraceContext currentTraceContext;
  @Autowired ThreadLocalSpan threadLocalSpan;

  @Bean ExecuteListenerProvider pgListener() {
    return new DefaultExecuteListenerProvider(this);
  }

  @Bean Executor pgExecutor() {
    return makeContextAware(
      new ZipkinPostgreSQLStorageConfiguration().pgExecutor(),
      currentTraceContext
    );
  }
  ...
  @Override public void renderEnd(ExecuteContext ctx) {
    ...
    span.remoteServiceName("postgresql");
    span.remoteIpAndPort(props.getHost(), props.getPort());
    span.start();
  }
