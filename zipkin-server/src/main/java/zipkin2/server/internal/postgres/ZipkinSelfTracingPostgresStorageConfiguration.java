...
/** Sets up the Postgres tracing in Brave as an initialization. */
@ConditionalOnSelfTracing
@ConditionalOnProperty(name = "zipkin.storage.type", havingValue = "postgres")
public class ZipkinSelfTracingPostgresStorageConfiguration extends DefaultExecuteListener {
  @Autowired ZipkinPostgresStorageProperties postgres;
  @Autowired CurrentTraceContext currentTraceContext;
  @Autowired ThreadLocalSpan threadLocalSpan;

  @Bean
  ExecuteListenerProvider postgresListener() {
  ...
  Executor postgresExecutor() {
    return makeContextAware(
      new ZipkinPostgresStorageConfiguration().postgresExecutor(),
      currentTraceContext
    );
  }
  ...
  @Override public void renderEnd(ExecuteContext ctx) {
    ...
    span.remoteServiceName("postgres");
    span.remoteIpAndPort(postgres.getHost(), postgres.getPort());
    span.start();
  }
