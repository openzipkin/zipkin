...
import com.twitter.zipkin.gen.{TraceCombo, ZipkinQuery, Adjust}
  ...
  val handleStatic =
    new Service[Request, Renderer] {
      private[this] val StaticRenderer =
        Future.value(MustacheRenderer("static.mustache", Map.empty[String, Object]))

      def apply(req: Request): Future[Renderer] = StaticRenderer
    }
