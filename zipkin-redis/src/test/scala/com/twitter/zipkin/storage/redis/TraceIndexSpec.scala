...
class TraceIndexSpec extends RedisSpecification {
  ...
  test("create/read") {
    ...
    result(index.list("key", 1, 1, 1)) should be(Seq(IndexedTraceId(1234, 1)))
  }

  test("list with multiple trace ids") {
    result(index.add("key", 1, 1234))
    result(index.add("key", 2, 4567))
    result(index.list("key", 100, 100, 100)) should be(Seq(
  ...
  test("list is unique on trace id") {
    ...
    result(index.list("key", 100, 100, 100)) should be(Seq(IndexedTraceId(1234, 2)))
  }

  test("list respects duration") {
    ...
    result(indexWithTtl.list("key", 35 * 1000000, Duration.fromSeconds(10).inMicroseconds, 100)) should be(Seq(
