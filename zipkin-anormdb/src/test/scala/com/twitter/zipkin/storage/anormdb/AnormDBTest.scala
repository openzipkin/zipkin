...
class AnormDBTest extends FunSuite {
  test("have the correct schema") {
    ...
    assert(expectedTables.forall { t => tables.contains(t) })
    ...
    assert(better_cols.forall(c => expectedCols.contains(c)))
    con.close()
  }

  test("insert and get rows") {
    implicit val con = new DB(new DBConfig("sqlite-memory", new DBParams(dbName = "zipkinTest"))).install()
    // Insert
    ...
    assert(result === List((2L, Some(1L), 1L, "mySpan", 1, Some(1000L), 0L)))
    con.close()
  }
