...
import { Constants, ConstantNames } from './trace-constants';
...
export default function detailedTraceSummary(trace, logsUrl = undefined) {
  const summary = traceSummary(trace);
  const { traceId } = summary;
  const durationStr = mkDurationStr(summary.duration);
  ...
  const spans = _(getRootSpans(trace)).flatMap(
    ...
    return {
      ...
      children: (groupByParentId[span.id] || []).map(s => s.id),
