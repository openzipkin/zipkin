...
import static zipkin.internal.v2.internal.V2SpanWriter.annotationSizeInBytes;
import static zipkin.internal.v2.internal.V2SpanWriter.endpointSizeInBytes;
...
public final class V1SpanWriter implements Buffer.Writer<Span> {
  @Override public int sizeInBytes(Span value) {
    ...
    if (value.localEndpoint() != null) {
      endpointSize = endpointSizeInBytes(value.localEndpoint());
    ...
    if (annotationCount > 0) {
      ...
      for (int i = 0, length = value.annotations().size(); i < length; i++) {
        sizeInBytes += annotationSizeInBytes(value.annotations().get(i), endpointSize);
      }
    }
  ...
  @Override public void write(Span value, Buffer b) {
    ...
    if (annotationCount > 0) {
      ...
      if (beginAnnotation) {
        writeAnnotation(Annotation.create(parsed.startTs, parsed.begin), endpointBytes, b);
        if (length > 0) b.writeByte(',');
      }
      for (int i = 0; i < length; ) {
        writeAnnotation(value.annotations().get(i++), endpointBytes, b);
        if (i < length) b.writeByte(',');
      }
      if (endAnnotation) {
        if (length > 0) b.writeByte(',');
        writeAnnotation(Annotation.create(parsed.endTs, parsed.end), endpointBytes, b);
      }
      b.writeByte(']');
    }
    int binaryAnnotationCount = value.tags().size();
  ...
  static Parsed parse(Span in) {
    ...
    if (in.kind() != null) {
      switch (in.kind()) {
        case CLIENT:
          parsed.remoteEndpointType = Constants.SERVER_ADDR;
          parsed.begin = "cs";
          parsed.end = "cr";
          break;
        case SERVER:
          parsed.remoteEndpointType = Constants.CLIENT_ADDR;
          parsed.begin = "sr";
          parsed.end = "ss";
          break;
        case PRODUCER:
          parsed.remoteEndpointType = Constants.MESSAGE_ADDR;
          parsed.begin = "ms";
          parsed.end = "ws";
          break;
        case CONSUMER:
