...
import zipkin.storage.elasticsearch.http.ElasticsearchHttpDependencyWriter;
import zipkin.storage.elasticsearch.http.ElasticsearchHttpTestGraph;
...
public class ElasticsearchDependenciesTest extends DependenciesTest {
  ...
  public ElasticsearchDependenciesTest() {
    this.storage = ElasticsearchHttpTestGraph.INSTANCE.storage.get();
  }
  ...
  @VisibleForTesting void writeDependencyLinks(List<DependencyLink> links, long timestampMillis) {
    ...
    try {
      ElasticsearchHttpDependencyWriter.writeDependencyLinks(storage.client(), links, index,
