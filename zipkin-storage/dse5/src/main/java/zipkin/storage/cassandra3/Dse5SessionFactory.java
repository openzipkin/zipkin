...
import zipkin.storage.cassandra3.Dse5Schema.EndpointUDT;
import zipkin.storage.cassandra3.Dse5Schema.TypeCodecImpl;

import static zipkin.storage.cassandra3.Dse5Schema.DEFAULT_KEYSPACE;

/**
 * Creates a session and ensures schema if configured. Closes the cluster and session if any
 * exception occurred.
 */
final class Dse5SessionFactory implements Dse5Storage.SessionFactory {

  /**
   * Creates a session and ensures schema if configured. Closes the cluster and session if any
   * exception occurred.
   */
  @Override public Session create(Dse5Storage cassandra) {
    ...
    try {
      ...
      if (cassandra.ensureSchema) {
        ...
        Dse5Schema.ensureExists(cassandra.keyspace, session);
  ...
  private static void initializeUDTs(Session session) {
    Dse5Schema.ensureExists(DEFAULT_KEYSPACE + "_udts", session);
  ...
  // Visible for testing
  static Cluster buildCluster(Dse5Storage cassandra) {
  ...
  static List<InetSocketAddress> parseContactPoints(Dse5Storage cassandra) {
