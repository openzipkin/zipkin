 ...
 *
 * <p>Dse5Schema is installed by default from "/dse5-schema.cql"
 */
// This component is named Dse5Storage as it correlates to "cassandra3" storage types, and
// makes health-checks more obvious. Note: this is the only public type in the package.
public final class Dse5Storage
    extends LazyGuavaStorageComponent<Dse5SpanStore, CassandraSpanConsumer> {

  // @FunctionalInterface, except safe for lower language levels
  public interface SessionFactory {
    Dse5SessionFactory DEFAULT = new Dse5SessionFactory();

    Session create(Dse5Storage storage);
  }
  ...
  public static final class Builder {
    String keyspace = Dse5Schema.DEFAULT_KEYSPACE;
    ...
    Dse5SessionFactory sessionFactory = Dse5SessionFactory.DEFAULT;

    /** Override to control how sessions are created. */
    public Builder sessionFactory(Dse5SessionFactory sessionFactory) {
      this.sessionFactory = checkNotNull(sessionFactory, "sessionFactory");
      return this;
    }

    /** Keyspace to store span and index data. Defaults to "dse5" */
    ...
    public Dse5Storage build() {
      return new Dse5Storage(this);
    }
  ...
  Dse5Storage(Builder builder) {
    ...
    this.session = new LazyCloseable<Session>() {
      @Override protected Session compute() {
        return sessionFactory.create(Dse5Storage.this);
      }
    };
  }
  ...
  @Override protected Dse5SpanStore computeGuavaSpanStore() {
    return new Dse5SpanStore(session.get(), maxTraceCols, indexFetchMultiplier);
  }
  ...
  @VisibleForTesting void clear() {
    ...
    for (String cf : ImmutableList.of(Dse5Schema.TABLE_TRACES,
        Dse5Schema.TABLE_TRACE_BY_SERVICE_SPAN,
        Dse5Schema.TABLE_DEPENDENCIES
    )) {
