...
import static zipkin.storage.cassandra3.Dse5SessionFactory.buildCluster;
import static zipkin.storage.cassandra3.Dse5SessionFactory.parseContactPoints;
import static org.assertj.core.api.Assertions.assertThat;
import static org.mockito.Mockito.mock;

public class Dse5SessionFactoryTest {

  @Test
  public void contactPoints_defaultsToLocalhost() {
    assertThat(parseContactPoints(Dse5Storage.builder().build()))
        .containsExactly(new InetSocketAddress("127.0.0.1", 9042));
  }

  @Test
  public void contactPoints_defaultsToPort9042() {
    assertThat(parseContactPoints(Dse5Storage.builder().contactPoints("1.1.1.1").build()))
  ...
  public void contactPoints_defaultsToPort9042_multi() {
    assertThat(parseContactPoints(
        Dse5Storage.builder().contactPoints("1.1.1.1:9143,2.2.2.2").build()))
        .containsExactly(
  ...
  public void contactPoints_hostAndPort() {
    assertThat(parseContactPoints(Dse5Storage.builder().contactPoints("1.1.1.1:9142").build()))
        .containsExactly(new InetSocketAddress("1.1.1.1", 9142));
  }

  @Test
  public void connectPort_singleContactPoint() {
    assertThat(buildCluster(Dse5Storage.builder().contactPoints("1.1.1.1:9142").build())
  ...
  public void connectPort_whenContactPointsHaveSamePort() {
    assertThat(
        buildCluster(Dse5Storage.builder().contactPoints("1.1.1.1:9143,2.2.2.2:9143").build())
            .getConfiguration().getProtocolOptions().getPort())
        .isEqualTo(9143);
  }
  ...
  public void connectPort_whenContactPointsHaveMixedPorts_coercesToDefault() {
    assertThat(
        buildCluster(Dse5Storage.builder().contactPoints("1.1.1.1:9143,2.2.2.2").build())
            .getConfiguration().getProtocolOptions().getPort())
        .isEqualTo(9042);
  }
  ...
  public void usernamePassword_impliesNullDelimitedUtf8Bytes() {
    Authenticator authenticator =
        buildCluster(Dse5Storage.builder().username("bob").password("secret").build())
  ...
  public void authProvider_defaultsToNone() {
    assertThat(buildCluster(Dse5Storage.builder().build())
        .getConfiguration().getProtocolOptions().getAuthProvider())
  ...
  public void loadBalancing_defaultsToRoundRobin() {
    RoundRobinPolicy policy = toRoundRobinPolicy(Dse5Storage.builder().build());
  ...
  RoundRobinPolicy toRoundRobinPolicy(Dse5Storage storage) {
  ...
  public void loadBalancing_settingLocalDcIgnoresOtherDatacenters() {
    DCAwareRoundRobinPolicy policy = toDCAwareRoundRobinPolicy(
        Dse5Storage.builder().localDc("bar").build());
  ...
  DCAwareRoundRobinPolicy toDCAwareRoundRobinPolicy(Dse5Storage storage) {
  ...
  public void maxConnections_defaultsTo8() {
    PoolingOptions poolingOptions = buildCluster(Dse5Storage.builder().build())
  ...
  public void maxConnections_setsMaxConnectionsPerDatacenterLocalHost() {
    PoolingOptions poolingOptions =
        buildCluster(Dse5Storage.builder().maxConnections(16).build())
            .getConfiguration().getPoolingOptions();
