...
public final class EnsureSchemaTest {
  ...
  @BeforeClass public static void checkCassandraIsUp() {
    try (Cluster cluster = buildCluster(Dse5Storage.builder().build());
  ...
  public void connectAndDropKeyspace() {
    ...
    cluster = closer.register(buildCluster(Dse5Storage.builder().keyspace(keyspace).build()));
  ...
  @Test public void installsKeyspaceWhenMissing() {
    Dse5Schema.ensureExists(keyspace, session);
