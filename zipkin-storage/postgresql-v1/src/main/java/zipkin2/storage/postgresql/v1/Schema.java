...
import static zipkin2.storage.postgresql.v1.internal.generated.tables.ZipkinDependencies.ZIPKIN_DEPENDENCIES;
...
final class Schema {
  ...
  <R extends Record> Condition traceIdCondition(
    TableField<R, Long> TRACE_ID_HIGH,
    TableField<R, Long> TRACE_ID, Set<Pair> traceIds
  ) {
    ...
    if (hasTraceIdHigh && strictTraceId) {
      ...
      for (Pair traceId128 : traceIds) {
        result[i++] = DSL.row(traceId128.left, traceId128.right);
      }
  ...
  static <T, R extends Record, TF extends TableField<R, T>> T maybeGet(R record, TF field, T defaultValue) {
