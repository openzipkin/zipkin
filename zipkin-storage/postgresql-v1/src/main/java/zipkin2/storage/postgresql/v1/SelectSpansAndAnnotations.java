...
import static zipkin2.storage.postgresql.v1.Schema.maybeGet;
...
abstract class SelectSpansAndAnnotations implements Function<DSLContext, List<Span>> {
  static final class Factory {
    final zipkin2.storage.postgresql.v1.Schema schema;
    final boolean strictTraceId;

    Factory(zipkin2.storage.postgresql.v1.Schema schema, boolean strictTraceId) {
      this.schema = schema;
    ...
    spansWithoutAnnotations =
      context
        ...
        .map(
          r ->
            V1Span.newBuilder()
              .traceIdHigh(maybeGet( (ZipkinSpansRecord) r, ZIPKIN_SPANS.TRACE_ID_HIGH, 0L))
              .traceId(r.getValue(ZIPKIN_SPANS.TRACE_ID))
              .name(r.getValue(ZIPKIN_SPANS.NAME))
              .id(r.getValue(ZIPKIN_SPANS.ID))
              .parentId(maybeGet( (ZipkinSpansRecord)r, ZIPKIN_SPANS.PARENT_ID, 0L))
              .timestamp(maybeGet( (ZipkinSpansRecord)r, ZIPKIN_SPANS.START_TS, 0L))
              .duration(maybeGet( (ZipkinSpansRecord)r, ZIPKIN_SPANS.DURATION, 0L))
      ...
      context
        ...
        .collect(
          groupingBy(
            (Record a) ->
              row(
                maybeGet( (ZipkinAnnotationsRecord) a, ZIPKIN_ANNOTATIONS.TRACE_ID_HIGH, 0L),
  ...
  static Endpoint endpoint(Record a) {
    Endpoint.Builder result =
      Endpoint.newBuilder()
        ...
        .port(maybeGet( (ZipkinAnnotationsRecord) a, ZIPKIN_ANNOTATIONS.ENDPOINT_PORT, (short) 0));
    int ipv4 = maybeGet( (ZipkinAnnotationsRecord) a, ZIPKIN_ANNOTATIONS.ENDPOINT_IPV4, 0);
    if (ipv4 != 0) {
    ...
    result.parseIp(maybeGet( (ZipkinAnnotationsRecord) a, ZIPKIN_ANNOTATIONS.ENDPOINT_IPV6, null));
    return result.build();
  }
}
