...
final class DependencyLinkV2SpanIterator implements Iterator<Span> {
  ...
  public Span next() {
    ...
    while (hasNext()) { // there are more values for this trace
      ...
      String key = emptyToNull( (ZipkinAnnotationsRecord) next, ZIPKIN_ANNOTATIONS.A_KEY);
      String value = emptyToNull( (ZipkinAnnotationsRecord) next, ZIPKIN_ANNOTATIONS.ENDPOINT_SERVICE_NAME);
    ...
    Long parentId = Schema.maybeGet( (ZipkinSpansRecord) row, ZipkinSpans.ZIPKIN_SPANS.PARENT_ID, (Long) 0L);
    Span.Builder result =
  ...
  static  @Nullable <R extends Record> String emptyToNull(R next, TableField<R, String> field) {
