...
final class SelectAutocompleteValues implements Function<DSLContext, List<String>> {
  ...
  @Override public List<String> apply(DSLContext context) {
    return context.selectDistinct(ZipkinAnnotations.ZIPKIN_ANNOTATIONS.A_VALUE)
      .from(ZipkinAnnotations.ZIPKIN_ANNOTATIONS)
      .where(ZipkinAnnotations.ZIPKIN_ANNOTATIONS.A_TYPE.eq(V1BinaryAnnotation.TYPE_STRING)
        .and(ZipkinAnnotations.ZIPKIN_ANNOTATIONS.A_KEY.eq(autocompleteKey)))
      .fetch(ZipkinAnnotations.ZIPKIN_ANNOTATIONS.A_VALUE, STRING_CONVERTER);
  }
