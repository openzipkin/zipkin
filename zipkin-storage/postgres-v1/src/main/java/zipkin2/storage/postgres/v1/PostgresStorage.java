...
public final class PostgresStorage extends StorageComponent {
  ...
  public static final class Builder extends StorageComponent.Builder {
    ...
    @Override public PostgresStorage build() {
  ...
  PostgresStorage(PostgresStorage.Builder builder) {
  ...
  @Override public SpanStore spanStore() {
    return new PostgresSpanStore(this, schema());
  }
  ...
  @Override public AutocompleteTags autocompleteTags() {
    return new PostgresAutocompleteTags(this, schema());
  }

  @Override public SpanConsumer spanConsumer() {
    return new PostgresSpanConsumer(dataSourceCallFactory, schema());
  }

  @Override public CheckResult check() {
    try (Connection conn = datasource.getConnection()) {
      context.get(conn).select(ZipkinSpans.ZIPKIN_SPANS.TRACE_ID).from(ZipkinSpans.ZIPKIN_SPANS).limit(1).execute();
    } catch (Throwable e) {
  ...
  @Override public final String toString() {
    return "PostgresStorage{datasource=" + datasource + "}";
  }
  ...
  void clear() {
    try (Connection conn = datasource.getConnection()) {
      context.get(conn).truncate(ZipkinSpans.ZIPKIN_SPANS).execute();
      context.get(conn).truncate(ZipkinAnnotations.ZIPKIN_ANNOTATIONS).execute();
      context.get(conn).truncate(ZipkinDependencies.ZIPKIN_DEPENDENCIES).execute();
