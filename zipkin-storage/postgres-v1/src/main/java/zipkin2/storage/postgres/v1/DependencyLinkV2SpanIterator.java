...
import static zipkin2.storage.postgres.v1.Schema.maybeGet;
...
final class DependencyLinkV2SpanIterator implements Iterator<Span> {
  ...
  public Span next() {
    ...
    while (hasNext()) { // there are more values for this trace
      ...
      String key = emptyToNull(next, ZipkinAnnotations.ZIPKIN_ANNOTATIONS.A_KEY);
      String value = emptyToNull(next, ZipkinAnnotations.ZIPKIN_ANNOTATIONS.ENDPOINT_SERVICE_NAME);
      ...
      switch (key) {
        ...
        case "error":
          ...
          error = V1BinaryAnnotation.TYPE_STRING == next.get(ZipkinAnnotations.ZIPKIN_ANNOTATIONS.A_TYPE);
      }
    }
