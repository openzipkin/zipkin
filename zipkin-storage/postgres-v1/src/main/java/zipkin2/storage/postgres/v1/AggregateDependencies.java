...
final class AggregateDependencies implements Function<DSLContext, List<DependencyLink>> {
  ...
  public List<DependencyLink> apply(DSLContext context) {
    ...
    SelectConditionStep<Record1<Long>> traceIDs = context.selectDistinct(ZipkinSpans.ZIPKIN_SPANS.TRACE_ID)
      .from(ZipkinSpans.ZIPKIN_SPANS)
      .where(startTsBegin == startTsEnd
        ? ZipkinSpans.ZIPKIN_SPANS.START_TS.lessOrEqual(startTsEnd)
        : ZipkinSpans.ZIPKIN_SPANS.START_TS.between(startTsBegin, startTsEnd));
    ...
    Cursor<Record> cursor = context.selectDistinct(schema.dependencyLinkerFields)
      ...
      .from(ZipkinSpans.ZIPKIN_SPANS.leftJoin(ZipkinAnnotations.ZIPKIN_ANNOTATIONS)
        ...
        .on(ZipkinSpans.ZIPKIN_SPANS.TRACE_ID.eq(ZipkinAnnotations.ZIPKIN_ANNOTATIONS.TRACE_ID)
          .and(ZipkinSpans.ZIPKIN_SPANS.ID.eq(ZipkinAnnotations.ZIPKIN_ANNOTATIONS.SPAN_ID)))
        .and(ZipkinAnnotations.ZIPKIN_ANNOTATIONS.A_KEY.in("lc", "cs", "ca", "sr", "sa", "ma", "mr", "ms", "error")))
      .where(ZipkinSpans.ZIPKIN_SPANS.TRACE_ID.in(traceIDs))
